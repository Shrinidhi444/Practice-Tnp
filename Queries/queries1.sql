...
/*select * from countries.countries order by r*/
...
WHERE table_schema = 'countries' AND table_name = 'regions';
...
(101, 'Alice Johnson', 'Manager', '2020-05-12', 75000.00),
(102, 'Bob Smith', 'Developer', '2021-07-19', 60000.50),
(103, 'Charlie Davis', 'Analyst', '2019-11-23', 55000.75),
(104, 'David White', 'HR', '2022-03-15', 50000.00),
(105, 'Emma Brown', 'Sales', '2018-09-07', 65000.25),
(106, 'Frank Harris', 'Manager', '2017-06-18', 80000.00),
(107, 'Grace Wilson', 'Developer', '2023-02-28', 62000.75),
(108, 'Henry Clark', 'Analyst', '2020-12-11', 58000.50),
(109, 'Ivy Adams', 'HR', '2019-08-05', 52000.00),
(110, 'Jack Lee', 'Sales', '2021-10-22', 67000.75),
(111, 'Kevin Martin', 'Manager', '2016-04-30', 90000.00),
(112, 'Lily Evans', 'Developer', '2018-11-09', 61000.50),
(113, 'Mike Scott', 'Analyst', '2022-07-15', 54000.75),
(114, 'Nancy Cooper', 'HR', '2015-03-25', 49000.00),
(115, 'Oscar Turner', 'Sales', '2017-12-14', 66000.25),
(116, 'Paul Walker', 'Manager', '2020-06-10', 78000.50),
(117, 'Quinn Lewis', 'Developer', '2019-09-21', 60000.75),
(118, 'Rachel Green', 'Analyst', '2023-01-05', 56000.00),
(119, 'Steve Rogers', 'HR', '2018-02-17', 51000.00),
(120, 'Tony Stark', 'Sales', '2016-07-04', 70000.25);

select JOB,ename,hiredate,sal from Emp; 

select ename as Employee_nsme, Job as Job_title,sal as Monthly_sal from emp;
...
/* create table [if not exist] schema.table(
column1 datatype(value/constraints)
.
;
*/
...
insert into temp_schema.table1
values
('Apple', 1234.56, '2024-02-15'),
('Banana', 789.01, '2023-12-10'),
('Cherry', 4567.89, '2022-08-25');
...
insert into temp_schema.table2
select * from temp_schema.table1;
...
/*update temp_schema.alter_demo1
set col3 = 40
where empid=3*/

update temp_schema.table2
set col_num = 800
where col_string = 'banana';
...
update temp_schema.table2
set col_num =500,col_date='2004-06-02'
where col_string='Apple';
...
delete from temp_schema.table2
where col_string='cherry';

use temp_schema;

create table temp_schema.cons_demo (col1 INT, col2 varchar(255));

insert into temp_schema.cons_demo
values 
(24,'shri'),
(30,'sak'),
(35,'urvi');

select * from temp_schema.cons_demo;

-- not null constraints
alter table temp_schema.cons_demo
